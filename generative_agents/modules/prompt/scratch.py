...
class Scratch:
    ...
    def _base_desc(self, existential_context=""):
    ...
    def prompt_poignancy_event(self, event, existential_context=""):
        prompt = self.build_prompt(
            "poignancy_event",
            {
                "base_desc": self._base_desc(existential_context),
                "agent": self.name,
    ...
    def prompt_poignancy_chat(self, event, existential_context=""):
        prompt = self.build_prompt(
            "poignancy_chat",
            {
                "base_desc": self._base_desc(existential_context),
    ...
    def prompt_wake_up(self, existential_context=""):
        prompt = self.build_prompt(
            "wake_up",
            {
                "base_desc": self._base_desc(existential_context),
    ...
    def prompt_schedule_init(self, wake_up, existential_context=""):
        prompt = self.build_prompt(
            "schedule_init",
            {
                "base_desc": self._base_desc(existential_context),
    ...
    def prompt_schedule_daily(self, wake_up, daily_schedule, existential_context=""):
        hourly_schedule = ""
    ...
    def prompt_schedule_decompose(self, plan, schedule, existential_context=""):
        def _plan_des(plan):
    ...
    def prompt_schedule_revise(self, action, schedule, existential_context=""):
    ...
    def prompt_determine_sector(self, describes, spatial, address, tile, existential_context=""):
    ...
    def prompt_determine_arena(self, describes, spatial, address, existential_context=""):
        prompt = self.build_prompt(
            "determine_arena",
            {
                "agent": self.name,
    ...
    def prompt_determine_object(self, describes, spatial, address, existential_context=""):
    ...
    def prompt_describe_emoji(self, describe, existential_context=""):
        prompt = self.build_prompt(
            "describe_emoji",
            {
                "action": describe,
    ...
    def prompt_describe_event(self, subject, describe, address, emoji=None, existential_context=""):
        prompt = self.build_prompt(
            "describe_event",
            {
                "action": describe,
    ...
    def prompt_describe_object(self, obj, describe, existential_context=""):
        prompt = self.build_prompt(
            "describe_object",
            {
                "object": obj,
    ...
    def prompt_decide_chat(self, agent, other, focus, chats, existential_context=""):
        def _status_des(a):
            event = a.get_event()
            if a.path:
    ...
    def prompt_decide_chat_terminate(self, agent, other, chats, existential_context=""):
    ...
    def prompt_decide_wait(self, agent, other, focus, existential_context=""):
    ...
    def prompt_summarize_relation(self, agent, other_name, existential_context=""):
    ...
    def prompt_generate_chat(self, agent, other, relation, chats, existential_context=""):
    ...
    def prompt_generate_chat_check_repeat(self, agent, chats, content, existential_context=""):
        conversation = "\n".join(["{}: {}".format(n, u) for n, u in chats])
        conversation = (
                conversation or "[对话尚未开始]"
        )
    ...
    def prompt_summarize_chats(self, chats, existential_context=""):
        conversation = "\n".join(["{}: {}".format(n, u) for n, u in chats])
    ...
    def prompt_reflect_focus(self, nodes, topk, existential_context=""):
        prompt = self.build_prompt(
            "reflect_focus",
            {
                "reference": "\n".join(["{}. {}".format(idx, n.describe) for idx, n in enumerate(nodes)]),
    ...
    def prompt_reflect_insights(self, nodes, topk, existential_context=""):
        prompt = self.build_prompt(
            "reflect_insights",
            {
                "reference": "\n".join(["{}. {}".format(idx, n.describe) for idx, n in enumerate(nodes)]),
    ...
    def prompt_reflect_chat_planing(self, chats, existential_context=""):
    ...
    def prompt_reflect_chat_memory(self, chats, existential_context=""):
        all_chats = "\n".join(["{}: {}".format(n, c) for n, c in chats])
    ...
    def prompt_retrieve_plan(self, nodes, existential_context=""):
        statements = [
    ...
    def prompt_retrieve_thought(self, nodes, existential_context=""):
        statements = [
            n.create.strftime("%Y-%m-%d %H:%M") + "：" + n.describe for n in nodes
        ]
    ...
    def prompt_retrieve_currently(self, plan_note, thought_note, existential_context=""):
        time_stamp = (
